...
export default function DeleteStudent({ student, onClose, onDelete, isLoading }) {
  ...
  const handleDelete = async () => {
    if (confirmText !== student.name) {
      alert("Please type the student's name correctly to confirm deletion.");
      return;
    }
        ...
        <div className="p-6">
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-linear-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white text-sm font-bold shadow-sm">
                  {student.avatar}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-200">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-700 truncate">{student.course}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">Nationality</p>
                  <p className="text-sm font-medium text-gray-700">{student.nationality}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">Academic Year</p>
                  <p className="text-sm font-medium text-gray-700">{student.academicYear}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-red-400/60 focus:border-transparent outline-none transition-all"
              disabled={isLoading || isDeleting}
            />
            {confirmText && confirmText !== student.name && (
